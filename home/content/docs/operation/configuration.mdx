...
MoEngage event sending configuration. When unset, MoEngage event sending is disabled.
...
Daily Slack summary of pending admin queue counts. When the webhook URL is unset, the summary job is disabled.

### `SLACK_QUEUE_SUMMARY_WEBHOOK_URL`
...
### `SLACK_QUEUE_SUMMARY_TIME`

<table>
<tr><td>type</td><td>`string`</td></tr>
<tr><td>default</td><td>`23:55`</td></tr>
</table>
...
### `SLACK_QUEUE_SUMMARY_TIMEZONE`

<table>
<tr><td>type</td><td>`string`</td></tr>
<tr><td>default</td><td>`Asia/Kolkata`</td></tr>
</table>

The IANA timezone name used to interpret `SLACK_QUEUE_SUMMARY_TIME`.

### `SLACK_QUEUE_SUMMARY_INTERVAL`
...
Daily job that folds likes/replies from the last 24 hours into each post's stored rank_score, since feed ranking no longer computes engagement live at read time. Runs unconditionally; only the schedule is configurable.

### `RANKER_JOB_TIME`

<table>
<tr><td>type</td><td>`string`</td></tr>
<tr><td>default</td><td>`06:30`</td></tr>
</table>

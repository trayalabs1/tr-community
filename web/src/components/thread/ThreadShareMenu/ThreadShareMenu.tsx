...
export function ThreadShareMenu({ thread }: Props) {
  ...
  const { selected, subtitle, initialShared, handlers } = useThreadShareMenu({
    thread,
    enabled: open,
  });
    ...
    <Menu.Content minW="64" userSelect="none">
      <Menu.ItemGroup id="share">
        ...
        {channels.map((c) => {
          ...
          const wasShared = initialShared.has(c.id);
          return (
            <Menu.CheckboxItem
              key={c.id}
              value={c.id}
...
function ConfirmShareButton({
  ...
  const toAdd = [...selected].filter((id) => !initialShared.has(id)).length;
  const toRemove = [...initialShared].filter((id) => !selected.has(id)).length;
  const hasChanges = toAdd > 0 || toRemove > 0;

  const label = () => {
    if (!hasChanges) return "No changes";
    if (toAdd > 0 && toRemove === 0) {
      return `Share to ${toAdd} channel${toAdd === 1 ? "" : "s"}`;
    }
    if (toRemove > 0 && toAdd === 0) {
      return `Unshare from ${toRemove} channel${toRemove === 1 ? "" : "s"}`;
    }

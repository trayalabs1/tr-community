...
// The ThreadShareFunc type is an adapter to allow the use of ordinary
// function as ThreadShare mutator.
type ThreadShareFunc func(context.Context, *ent.ThreadShareMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ThreadShareFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ThreadShareMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ThreadShareMutation", m)
}
